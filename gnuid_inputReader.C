...
void GnuidInputReader::read_mesh(Mesh& mesh, const std::string& working_directory)
{
...
void GnuidInputReader::read_es(EquationSystems& equation_systems, const std::string& working_directory)
{
  GetPot input_file(working_directory + "/" + "gnuid.in");
  ...
  // Start reading boundary conditions sections
  ...
  // Dirichlet flow defective boundary conditions
  if (input_file.have_variable("n_flow_defective_boundaries"))
  {
      equation_systems.parameters.set<unsigned int>("n flow bound") = input_file("n_flow_defective_boundaries", 0);
      identified_variables.push_back("n_flow_defective_boundaries");
  }
  else std::cerr<<"Missing variable n_flow_defective_boundaries, using default value. Check your input file"<<std::endl;
  unsigned int n_flow_bound = equation_systems.parameters.get<unsigned int>("n flow bound");
  for (unsigned int i = 0; i < n_flow_bound; i++)
  {
      char flow_bc_type[1024];
      char flow_bc_name[1024];
      char flow_bc_name_output[1024];
      ...
      sprintf(flow_bc_type,"flow_bc_%02d_type",i);
      sprintf(flow_bc_id,"flow_bc_%02d_id",i);
      sprintf(flow_bc_center,"flow_bc_%02d_center",i);
      sprintf(flow_bc_radius,"flow_bc_%02d_radius",i);
      sprintf(flow_bc_normal,"flow_bc_%02d_normal",i);
      sprintf(flow_bc_scaling,"flow_bc_%02d_scaling",i);
      sprintf(flow_bc_name,"flow_bc_%02d/",i);
      sprintf(flow_bc_name_output,"flow_bc_%02d",i);
      sprintf(flow_bc_n_fourier_modes,"flow_bc_%02d_n_fourier_modes",i);
      
      std::string prefix = std::string(flow_bc_name);
      input_file.set_prefix(flow_bc_name);
      if (input_file.have_variable("inlet_outlet"))
      {
         equation_systems.parameters.set<std::string>(flow_bc_type) = input_file("inlet_outlet", "inlet");
         identified_variables.push_back(prefix + "inlet_outlet");
      }
      else std::cerr<<"Missing variable inlet_outlet in section ["<<flow_bc_name_output
                    <<"], using default value. Check your input file sections and variables."<<std::endl;
      if (input_file.have_variable("flow_bc_id"))
      {
         equation_systems.parameters.set<unsigned short>(flow_bc_id) = input_file("flow_bc_id", 0);
         identified_variables.push_back(prefix + "flow_bc_id");
      }
      else std::cerr<<"Missing variable flow_bc_id in section ["<<flow_bc_name_output
                    <<"], using default value. Check your input file sections and variables."<<std::endl;
      if (input_file.have_variable("flow_bc_center_x") && input_file.have_variable("flow_bc_center_y") && input_file.have_variable("flow_bc_center_z"))
      {
         equation_systems.parameters.set<Point>(flow_bc_center) =
                                        Point(input_file("flow_bc_center_x",0.0),input_file("flow_bc_center_y",0.0),input_file("flow_bc_center_z",0.0));
         identified_variables.push_back(prefix + "flow_bc_center_x");
         identified_variables.push_back(prefix + "flow_bc_center_y");
         identified_variables.push_back(prefix + "flow_bc_center_z");
      }
      else std::cerr<<"Missing variable flow_bc_center in section ["<<flow_bc_name_output
                    <<"], using default value. Check your input file sections and variables."<<std::endl;
      if (input_file.have_variable("flow_bc_radius"))
      {
         equation_systems.parameters.set<Real>(flow_bc_radius) = input_file("flow_bc_radius", 0.0);
         identified_variables.push_back(prefix + "flow_bc_radius");
      }
      else std::cerr<<"Missing variable flow_bc_radius in section ["<<flow_bc_name_output
                    <<"], using default value. Check your input file sections and variables."<<std::endl;
      if (input_file.have_variable("flow_bc_normal_i") && input_file.have_variable("flow_bc_normal_j") && input_file.have_variable("flow_bc_normal_k"))
      {
         equation_systems.parameters.set<VectorValue<Real> >(flow_bc_normal) =
                                        VectorValue<Real>(input_file("flow_bc_normal_i",0.0),input_file("flow_bc_normal_j",0.0),input_file("flow_bc_normal_k",0.0)).unit();
         identified_variables.push_back(prefix + "flow_bc_normal_i");
         identified_variables.push_back(prefix + "flow_bc_normal_j");
         identified_variables.push_back(prefix + "flow_bc_normal_k");
      }
      else std::cerr<<"Missing variable flow_bc_normal in section ["<<flow_bc_name_output
                    <<"], using default value. Check your input file sections and variables."<<std::endl;
      if (input_file.have_variable("scaling"))
      {
         equation_systems.parameters.set<Real>(flow_bc_scaling) = input_file("scaling", 1.0);
         identified_variables.push_back(prefix + "scaling");
      }
      else std::cerr<<"Missing variable scaling in section ["<<flow_bc_name_output
                    <<"], using default value. Check your input file sections and variables."<<std::endl;
      if (input_file.have_variable("n_fourier_modes"))
      {
         equation_systems.parameters.set<unsigned int>(flow_bc_n_fourier_modes) = input_file("n_fourier_modes", 1);
         identified_variables.push_back(prefix + "n_fourier_modes");
      }
      else std::cerr<<"Missing variable n_fourier_modes in section ["<<flow_bc_name_output
                    <<"], using default value. Check your input file sections and variables."<<std::endl;
      for (unsigned int n = 0; n < equation_systems.parameters.get<unsigned int>(flow_bc_n_fourier_modes); n++)
      {
          char flow_bc_f_mode[1024];
          char f_mode_re[1024];
          char f_mode_im[1024];
          sprintf(flow_bc_f_mode,"flow_bc_%02d_f_mode_%02d",i,n);
          sprintf(f_mode_re,"f_mode_%02d_re",n);
          sprintf(f_mode_im,"f_mode_%02d_im",n);
          std::string f_m_re = std::string(f_mode_re);
          std::string f_m_im = std::string(f_mode_im);
          if (input_file.have_variable(f_mode_re) && input_file.have_variable(f_mode_im))
          {
             equation_systems.parameters.set<Complex>(flow_bc_f_mode) = Complex(input_file(f_mode_re, 0.0),input_file(f_mode_im, 0.0));
             identified_variables.push_back(prefix + f_m_re);
             identified_variables.push_back(prefix + f_m_im);
          }
          else std::cerr<<"Missing variable "<<f_mode_re<<" or "<<f_mode_im<<" in section ["<<flow_bc_name_output
                        <<"], using default value. Check your input file sections and variables."<<std::endl;
      }
      input_file.set_prefix("");
  }

  // Neumann "stress-free" boundary conditions
  ...
  // End reading boundary conditions sections
